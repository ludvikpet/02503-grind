...
def rescale_object(object_im: np.ndarray, face_width: int) -> np.ndarray: 
    ...
    """
    """
    Exercise 9: change the width_scale_factor such that the function adaptively fits the object onto the head
    """
    width_scale_factor = 0.2 #update this line such that the rescaling is fitting
...
if __name__=="__main__":
    ...
    n_frames = 0
    ROTATE = False #set to true if we want to rotate the object of interest with an angle - first in exercise 10
    stop = False
    N_eyes = 0
    ...
    while not stop:
        ...
        for i, (x,y,w,h) in enumerate(faces):
            ...
            if N_eyes==2: #only go further if we have detected two eyes.. 
                ...
                """
                Exercise 6: find the normalized normal vector
                """
                vec = 0 #find left to right eye vector as a np.array
                n_vec = 0 #find normal vector as a np.array
                norm = 0 #find normalizing constant for normal vector 
                ...
                if norm == 0:  #becomes zero if the same eye is detected twice - we can't place a hat then
                    ...
                    """
                    Exercise 7: calculate the end-points of the hat, i.e. where it should be placed in the image
                    """
                    end_point_hat_x = 50
                    end_point_hat_y = 50 #subtract because image coordinate system 
                    ...
                    """
                    Exercise 8: rescale size such that it fits the head-width  - implement rescale_object function
                    """
                    object_im_trf = rescale_object(object_im, w+100) #finish the implementation of this function

                    """
                    Exercise 9: define the coordinates for alignment in the local hat coordinate frame
                    """
                    anchor_y = 0
                    anchor_x = 0
                    ...
                    if ROTATE:      
                        """
                        Exercise 10: calculate the rotation angle which the hat should be rotated.
                        """              
                        theta = 0.0 #calculate the correct rotation
