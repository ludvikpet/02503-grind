...
def load_cascades(cascade_directory: str) -> Tuple[cv2.CascadeClassifier, cv2.CascadeClassifier]:
    ...
    #TODO EXERCISE: try the other eyeglasses version 
    #eyes_cascade_name = pretrained_dir + "/haarcascade_eye_tree_eyeglasses.xml" #seems to be more robust to lighting and has less false detections
...
def load_png_object(file_path: str) -> Tuple[np.ndarray, Tuple[int, int, int]]:
    """
    Loads a png object, such as a hat. Note, the .png should have a transparent background and be loaded with an alpha channel
...
def rotate_object(object_im: np.ndarray, angle: float, anchor_point: tuple, allow_resize: bool = False) -> Tuple[np.ndarray,np.ndarray,int,int] | Tuple[np.ndarray,np.ndarray]:
    ...
    Returns the difference in image size before and after rotation if allow_resize = True.  
    ...
    Returns:
        Tuple[np.ndarray,np.ndarray,int,int] | Tuple[np.ndarray,np.ndarray]: 
    """
    ...
    if allow_resize:
        #calculate the difference in sizes between original image and rotated image. Used for correcting position on top of head.
        dx = w-object_im.shape[1]
        dy = h-object_im.shape[0]
        return object_im, new_anchor, dx, dy 
    else:
        return object_im, new_anchor
...
if __name__=="__main__":
    ...
    n_vec_ref = np.array([0,0]).astype(int) #reference normal vector for time smoothing. Note: Could use a prior
    anchor_ref = 50*np.ones((2)).astype(int)
    end_point_ref = np.zeros((2)).astype(int)
    theta_ref = 0.0 #np.pi/2
    anchor_point = None 
    object_im_trf = None
    ...
    while not stop:
        ...
        for i, (x,y,w,h) in enumerate(faces):
            ...
            if N_eyes>=2: #case: pair of eyes could be detected
                ...
                #Exercise 6: calculate a vector going from left eye to right eye
                ...
                if norm == 0:  #becomes zero if the same eye is detected twice
                    ...
                    if SMOOTHEN:
                        if anchor_point is None or object_im_trf is None: #case: no initialization yet
                            ...
                            object_im_trf, anchor_point_new, dx, dy  = rotate_object(object_im_trf, theta_ref, anchor_point )
                        anchor_point[0] = anchor_point[0]-dy #- anchor_point_new[0]
                        anchor_point[1] = anchor_point[1]-dx #- anchor_point_new[1]
                        frame = insert_hat_on_frame(frame, object_im_trf, (end_point_ref[0],end_point_ref[1]), anchor_point)
                        N_skipped += 1
                    ...
                    if SMOOTHEN: 
                        end_point_arr = time_smoothen_detections(np.array([end_point_hat_x,end_point_hat_y]),end_point_ref,alpha=ALPHA).astype(int)
                        end_point_hat_x_org = end_point_hat_x
                        end_point_hat_y_org = end_point_hat_y
                        end_point_hat_x = end_point_arr[0]
                        end_point_hat_y = end_point_arr[1]
                        cv2.circle(frame,(end_point_hat_x,end_point_hat_y),10, color=(0,0,255),thickness=3, lineType=8, shift=0) 
                    ...
                    #rescale size such that it fits the head-width 
                    object_im_trf = rescale_object(object_im, w+100)
                    ...
                    #find the rotation angle as compared to horizontal (dy = 0) 
                    #theta = np.rad2deg(np.arctan(-vec[1]/vec[0]))
                    ...
                    if SMOOTHEN: 
                        theta = time_smoothen_detections(theta,theta_ref, alpha=0.95)
                    
                    #if np.abs(theta)>threshold: for saving compute
                    object_im_trf, anchor_point_new, dx, dy = rotate_object(object_im_trf, theta, anchor_point) #get the corresponding anchor point after applying rotation
                    ...
                    #np.savetxt(fname=f"frame_data/{n_frames}.txt",X=np.array([end_point_hat_x,end_point_hat_y]))
                    ...
                    if SMOOTHEN:
                        theta_ref = theta 
                        #anchor_ref = anchor_point 
                        end_point_ref = end_point_arr 
                        N_skipped = 0
            else: #case: no valid detection, fall-back to references 
                #rescale size such that it fits the head-width 
                #continue 
                if SMOOTHEN:
                    print("no obj detect hello")
                    if object_im_trf is None: #case: no initialization yet
                        object_im_trf = rescale_object(object_im, w+100)
                        anchor_point = np.array([object_im_trf.shape[0]-1,object_im_trf.shape[1]//2]) #y,x always 
                        object_im_trf, anchor_point_new, dx, dy  = rotate_object(object_im_trf, theta_ref, anchor_point )
                        anchor_point[0] = anchor_point[0]-dy #- anchor_point_new[0]
        ...
        print(f"{theta_ref}:{end_point_ref}")
