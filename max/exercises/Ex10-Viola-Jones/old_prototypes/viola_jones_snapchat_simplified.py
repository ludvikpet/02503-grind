...
def insert_hat_on_frame(frame_rgb, obj, coords, anchor, anchor_rot):
    ...
    anchor_y, anchor_x = anchor  # anchor of landmark-point inside the hat image before rotation
    anchor_rot_y, anchor_rot_x = anchor_rot
    ...
    #this works when there is no rotation
...
def rotate_object(object_im: np.ndarray, angle: float, anchor_point: tuple) -> np.ndarray:
    ...
    return object_im, new_anchor, dx, dy #new_anchor
        ...
        for (x,y,w,h) in faces:
            ...
            if N_eyes>=2: #case: pair of eyes could be detected
                ...
                if norm == 0:  #becomes zero if the same eye is detected twice
                    ...
                    object_im_trf, anchor_point_new, dx, dy = rotate_object(object_im_trf, theta, anchor_point) #get the corresponding anchor point after applying rotation. This is the coordinate which needs to be aligned with the end-points
                    print(f"anchor pt before update: {anchor_point}") 
                    ...
                    #anchor_point_new[1] = anchor_point_new[1] - object_im_trf.shape[1]//2
                    #anchor_point_new[0] = anchor_point_new[0] - (object_im_trf.shape[0]-1)
                    
                    print(f"anchor pt after update: {anchor_point_new}") 
                    ...
                    frame = insert_hat_on_frame(frame, object_im_trf, (end_point_hat_y,end_point_hat_x), anchor_point,anchor_point_new)
