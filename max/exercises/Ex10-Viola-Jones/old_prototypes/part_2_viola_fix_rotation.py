...
def rotate_object(object_im, angle, anchor_point, allow_resize=False):

    h, w = object_im.shape[:2]

    # rotate image using skimage
    rot_im = rotate(object_im, angle, center=(0,0), resize=allow_resize, order=1, preserve_range=True)

    # rotation matrix inverse (as skimage uses for bbox)
    ...
    # skimage uses tform.inverse(corners)
    corners_inv = (Rinv @ corners.T).T

    min_x = corners_inv[:,0].min()
    min_y = corners_inv[:,1].min()

    # resize translation skimage applies
    translate = np.array([-min_x, -min_y])

    # forward-rotate anchor (points forward transform)
    R = np.array([
        [ np.cos(theta),  np.sin(theta)],
        [-np.sin(theta),  np.cos(theta)]
    ])
    anchor = np.array(anchor_point)
    anchor_rot = R @ anchor + translate

    # return in (y,x)
    return rot_im.astype(object_im.dtype), np.array([anchor_rot[1], anchor_rot[0]])
            ...
            if N_eyes==2: #only go further if we have detected two eyes.. 
                ...
                if norm == 0:  #becomes zero if the same eye is detected twice - we can't place a hat then
                    ...
                    if ROTATE:      
                        ...
                        object_im_trf, anchor_point = rotate_object(object_im_trf, theta, anchor_point, allow_resize=True)
